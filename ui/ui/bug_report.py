...
def _save_bug_report(
    ...
    try:
        ...
        print(f"Bug report submission failed ({exc.code}): {details}", flush=True)
        return "❌ The report could not be saved. Please check the bug_reports table and its permissions."
    except Exception as exc:
        print(f"Bug report submission failed: {exc}", flush=True)
        return "❌ The report could not be saved. Please try again."


def build_bug_report_page(workspace_name: Any, visible: bool = False):
    """Build and return the Channel Coach bug-report page."""
    with gr.Column(visible=visible) as page:
        # Gradio event inputs must be components. Some callers pass a gr.State,
        # while others pass the workspace name as an ordinary string.
        workspace_input = (
            workspace_name
        ...
        submit_button = gr.Button("Submit bug report", variant="primary")
        result = gr.Markdown()

        submit_button.click(
            fn=_save_bug_report,
            inputs=[workspace_input, title, category, priority, description, steps],
            outputs=result,
        )
...
print("Loaded ui.bug_report with build_bug_report_page", flush=True)
